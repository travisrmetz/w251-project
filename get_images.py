...
class StellariumToImages:
    __args = None
    __frame_folder = None
    __script = """
    // Revamped from original code done by Ingo Berg
    ...
    function getImages(date, file_prefix, caption, hours, long, lat, alt, azi)
    {
        ...
        core.setObserverLocation(long, lat, 425, 1, "Ocean", "Earth");
        core.wait(0.5);

        //set altitude and direction
    ...
    def create_script(self):
        ...
        # writing script into folder
        file = open("{0}/.stellarium/scripts/get_sky.ssc".format(Path.home()), "w")
        file.write(script)
        file.close()
...
def main():
    parser = argparse.ArgumentParser("Get_Sky - A collector of sky images")
    parser.add_argument("-long", "--Longitude", dest="long", help='Longitude', default=-70.000, type=float)
    parser.add_argument("-lat", "--Latitude", dest="lat", help='Latitude', default=41.000, type=float)
    parser.add_argument("-alt", "--Altitude", dest="alt", help='Altitude of the center of the field of view', default=0, type=float)
    parser.add_argument("-az", "--Azimuth", dest="az", help='Azimut in degrees (View direction)', default=0, type=float)
    parser.add_argument("-d", "--Date",dest="date", help=' date ', default='2020-05-25', type=valid_date)
    ...
    print("Get_Sky - A collector of sky images:")
    print("-------------------------------------------")
    print("Python Version: {0}".format(sys.version_info))
    print("Date: {0}".format(args.date.strftime("%Y-%m-%d")))
